...
def paint_logo
  ...
  lines = @font.write('uDoc').split("\n")
  print @pastel.green(lines_at(lines, center_x, center_y - 5).join)
  lines = @reg_font.write("Thanks  For  Using", letter_spacing: 1).split("\n")
  print lines_at(lines, center_x, center_y).join
  lines = @font.write('APP').split("\n")
  print @pastel.green(lines_at(lines, center_x, center_y + 4).join)
end
